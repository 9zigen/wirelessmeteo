...
{{ :projects:wiki_noimage.png?direct&700 }}

  * Платформы: Teensy, Arduino Uno
  * Языки программирования: [[wp>Wiring_(development_platform) | Wiring (C++)]]
  * Проект на [[wpru>GitHub]]: http://github.com/amperka-projects/wireless-meteo
...
получающего эти показатели и отправляющего их на сервис «народный мониторинг».
...
===== Как это собрать? =====
  ...
  - Подключаем дисплей к контакту ''3'' тройка-шилда. При подключении всех тройка-модулей следите за тем, чтобы чёрный провод соединялся с контактом ''GND''. {{ :projects:wiki_noimage.png?direct&700 }}
  - Теперь подключаем силовой ключ к контакту ''5''. {{ :projects:wiki_noimage.png?direct&700 }}
  - У помпы нет контактов на концах проводов, поэтому приходится пользоваться клеммником. Если вы умеете паять, правильней было бы припаять к проводам [[amp>product/pin-headers|штырьковые контакты]]. {{ :projects:wiki_noimage.png?direct&700 }}
  - Полностью подключённое питание помпы выглядит так: {{ :projects:wiki_noimage.png?direct&700 }}
  -Теперь подключаем сенсор влажности почвы. {{ :projects:wiki_noimage.png?direct&700 }}
  - [[arduino-быстрый-старт:start|При помощи Arduino IDE]] программируем Arduino Uno скетчем [[projects:irrigator?&#скетч_для_arduino|irrigator.ino]].
  - Втыкаем сенсор влажности в землю.
  - Втыкаем конец трубки с водой в землю. Если горшок с землёй весит менее 2 кг, рекомендуется отдельно закрепить трубку, чтобы она не перевернула растение.
  - Опускаем помпу в ёмкость с водой.
  - Подключаем питание.
